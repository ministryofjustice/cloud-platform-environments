...
test_allow_if_valid_service_pod_and_irsa if {
	analysis.allow with input as sp_mock_tfplan
}
...
test_deny_if_namespace_mismatch if {
	...
	not analysis.allow with input as {
...
test_deny_if_involve_other_resource_change if {
	...
	not analysis.allow with input as {"resource_changes": [modified_plan, sp_mock_tfplan.resource_changes[_]]}
}
