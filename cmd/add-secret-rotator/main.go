...
var (
	...
	commitMessagePrefix = "fix(namespaces)Add SECRET_ROTATE_BLOCK from "
	baseBranch          = "main"
...
func main() {
	...
	commitBranch := "add-secret-rotator"
	...
	if err := createPR(commitBranch); err != nil {
...
func getTree(ref *github.Reference, namespace string) (tree *github.Tree, err error) {
	...
	file, err := os.Create(fmt.Sprintf("%s/%s/%s", filePathBase, namespace, "SECRET_ROTATE_BLOCK"))
	if err != nil {
		log.Fatal(err)
	}
	defer file.Close()
	fmt.Println("File to Add", secretRotatorFile)
	content, err := os.ReadFile(secretRotatorFile)
	if err != nil {
		return nil, err
	}
	entries = append(entries, &github.TreeEntry{Path: github.String(secretRotatorFile), Type: github.String("blob"), Content: github.String(string(content)), Mode: github.String("100644")})
...
func createPR(commitBranch string) (err error) {
	prTitle := "Add skip file to test namespaces"
	prDescription := "Add skip file to test namespaces"
