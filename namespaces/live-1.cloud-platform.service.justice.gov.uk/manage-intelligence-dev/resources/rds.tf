...
data "aws_iam_policy_document" "manage_intelligence_dev_rds_to_s3_export_policy" {
  ...
  statement {
    ...
    resources = [
      "${module.manage_intelligence_rds_to_s3_bucket.bucket_arn}",
      "${module.manage_intelligence_rds_to_s3_bucket.bucket_arn}/*",
      "arn:aws:s3:::mojap-land/hmpps/manage_intelligence/",
      "arn:aws:s3:::mojap-land/hmpps/manage_intelligence/*"
    ]
  }
}
...
resource "aws_iam_user" "user" {
  name = "manage-intelligence-rds-to-s3-snapshots-user-${random_id.id.hex}"
  path = "/system/manage-intelligence-rds-to-s3-snapshots-user/"
}
...
resource "aws_iam_user_policy" "policy" {
  name   = "manage-intelligence-rds-to-s3-snapshots-read-write"
  policy = data.aws_iam_policy_document.manage_intelligence_dev_rds_to_s3_export_policy.json
  user   = aws_iam_user.user.name
}
