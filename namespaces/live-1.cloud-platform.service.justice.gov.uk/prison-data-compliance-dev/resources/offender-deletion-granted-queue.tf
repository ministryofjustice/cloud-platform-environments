module "off_del_granted_queue" {
  ...
  sqs_name                   = "off_del_granted_queue"
...
resource "aws_sqs_queue_policy" "off_del_granted_queue_policy" {
  queue_url = module.off_del_granted_queue.sqs_id

  policy = <<EOF
  {
    "Version": "2012-10-17",
    "Id": "${module.off_del_granted_queue.sqs_arn}/SQSDefaultPolicy",
    "Statement":
      [
        {
          "Sid": "PublishPolicy",
          "Effect": "Allow",
          "Principal": {"AWS": "*"},
          "Resource": "${module.off_del_granted_queue.sqs_arn}",
...
module "offender_deletion_granted_dead_letter_queue" {
  ...
  infrastructure-support = var.infrastructure-support
  application            = var.application
  sqs_name               = "off_del_granted_queue_dl"
  encrypt_sqs_kms        = "true"
...
resource "kubernetes_secret" "off_del_granted_queue" {
  metadata {
  ...
  data = {
    access_key_id     = module.off_del_granted_queue.access_key_id
    secret_access_key = module.off_del_granted_queue.secret_access_key
    sqs_ocne_url      = module.off_del_granted_queue.sqs_id
    sqs_ocne_arn      = module.off_del_granted_queue.sqs_arn
    sqs_ocne_name     = module.off_del_granted_queue.sqs_name
  }
}
