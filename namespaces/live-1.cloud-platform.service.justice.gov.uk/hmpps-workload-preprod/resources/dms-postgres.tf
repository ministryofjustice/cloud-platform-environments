module "hmpps-workload-dms-postgres" {
...
resource "random_id" "dms_postgres_rand" {
  byte_length = 8
}

resource "aws_dms_endpoint" "source_sqlserver" {
  endpoint_id                 = "${var.team_name}-src-${random_id.dms_postgres_rand.hex}"
...
resource "aws_dms_endpoint" "target_postgres" {
  endpoint_id                 = "${var.team_name}-dstmssql-${random_id.dms_postgres_rand.hex}"
...
resource "aws_dms_replication_task" "replication_task_postgres" {
  ...
  replication_instance_arn = module.hmpps-workload-dms-postgres.replication_instance_arn
  replication_task_id      = "${var.team_name}-repl-${random_id.dms_postgres_rand.hex}"
...
resource "kubernetes_secret" "dms_instance_postgres" {
  ...
  data = {
    replication_instance_arn = module.hmpps-workload-dms-postgres.replication_instance_arn
    access_key_id            = module.hmpps-workload-dms-postgres.access_key_id
    secret_access_key        = module.hmpps-workload-dms-postgres.secret_access_key
