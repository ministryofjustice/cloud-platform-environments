module "hmpps-workload-preprod-s3-extract-bucket" {
    ...
    "Statement": [
      {
        ...
        "Condition": {
          "ArnEquals": { "aws:SourceArn": "${module.hmpps-workload-preprod-s3-extract-bucket.bucket_arn}" }
        }
      }
    ]
  }
...
resource "aws_s3_bucket_notification" "hmpps_workload_s3_notification" {
  bucket = module.hmpps-workload-preprod-s3-extract-bucket.bucket_name
...
resource "kubernetes_secret" "hmpps-workload-preprod-s3-extract-bucket" {
  metadata {
