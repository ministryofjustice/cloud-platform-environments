# Use default ingress controller NLB for prod
data "aws_lb" "ingress_default_prod_nlb" {
  tags = {
    "kubernetes.io/service-name" = "ingress-controllers/nginx-ingress-default-controller"
...
data "aws_network_interfaces" "nlb_enis" {
  filter {
    name   = "description"
    values = ["ELB ${data.aws_lb.ingress_default_prod_nlb.arn_suffix}"]
  }
  filter {
    name   = "vpc-id"
    values = [data.aws_lb.ingress_default_prod_nlb.vpc_id]
  }
}
...
# VPC Link for API Gateway - Production
# Production uses its own dedicated prod NLB, so it needs its own VPC Link
resource "aws_api_gateway_vpc_link" "api_gateway_vpc_link" {
  name        = "${var.namespace}-vpc-link"
  description = "VPC Link for ${var.namespace} API Gateway to Production NLB"
  target_arns = [data.aws_lb.ingress_default_prod_nlb.arn]
...
# apiGatewayIngress in values-preprod.yaml must have this same hostname
...
resource "aws_api_gateway_domain_name" "api_gateway_fqdn" {
  ...
  # No mTLS for launchpad-preprod by default (omit mutual_tls_authentication)
  depends_on = [
