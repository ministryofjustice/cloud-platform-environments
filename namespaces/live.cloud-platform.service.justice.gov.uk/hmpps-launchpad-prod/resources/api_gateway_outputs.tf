...
output "prod_nlb_ip_addresses" {
  description = "IP addresses of the production NLB - Add these to Ingress Allow List in values-prod.yaml"
  value = toset(flatten([
...
output "prod_nlb_arn" {
  description = "ARN of the production NLB used by VPC Link"
  value       = data.aws_lb.ingress_default_prod_nlb.arn
}

output "prod_nlb_dns_name" {
  description = "DNS name of the production NLB"
  value       = data.aws_lb.ingress_default_prod_nlb.dns_name
}

output "allowlist_cidr_blocks" {
  description = "CIDR blocks to add to Ingress Allow List (for use in values-prod.yaml)"
  value = [
