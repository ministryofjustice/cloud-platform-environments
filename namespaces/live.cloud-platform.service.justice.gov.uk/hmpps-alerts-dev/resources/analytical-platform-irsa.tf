module "ap_irsa" {
  ...
  # IRSA configuration
  service_account_name = "hmpps-alerts-to-ap-s3"
...
module "ap_service_pod" {
  ...
  # Configuration
  namespace            = var.namespace
  service_account_name = module.ap_irsa.service_account.name
}
...
data "aws_iam_policy_document" "ap_access" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:s3:::moj-reg-dev/landing/hmpps-alerts-dev/*",
      "arn:aws:s3:::moj-reg-dev/landing/hmpps-alerts-dev/"
    ]
  }
}
...
resource "kubernetes_secret" "ap_aws_secret" {
  ...
  data = {
    destination_bucket = "s3://moj-reg-dev/landing/hmpps-alerts-dev/"
