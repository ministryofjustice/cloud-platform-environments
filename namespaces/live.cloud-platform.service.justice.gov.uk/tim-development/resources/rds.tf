...
module "rds" {
  ...
  db_engine         = "postgres"
  db_engine_version = "16.9"   # If you are managing minor version updates, refer to user guide: https://user-guide.cloud-platform.service.justice.gov.uk/documentation/deploying-an-app/relational-databases/upgrade.html#upgrading-a-database-version-or-changing-the-instance-type
...
module "rds_3" {
  ...
  # PostgreSQL specifics
  db_engine         = "postgres"
  db_engine_version = "16.4"   # If you are managing minor version updates, refer to user guide: https://user-guide.cloud-platform.service.justice.gov.uk/documentation/deploying-an-app/relational-databases/upgrade.html#upgrading-a-database-version-or-changing-the-instance-type
  rds_family        = "postgres16"
  db_instance_class = "db.t4g.micro"
...
resource "kubernetes_secret" "rds_2" {
  metadata {
    name      = "rds2-postgresql-instance-output"
    namespace = var.namespace
  }

  data = {
    rds_instance_endpoint = module.rds_2.rds_instance_endpoint
    database_name         = module.rds_2.database_name
    database_username     = module.rds_2.database_username
    database_password     = module.rds_2.database_password
    rds_instance_address  = module.rds_2.rds_instance_address
  }
  /* You can replace all of the above with the following, if you prefer to
     * use a single database URL value in your application code:
     *
...
resource "kubernetes_config_map" "rds_2" {
  metadata {
    name      = "rds2-postgresql-instance-output"
    namespace = var.namespace
  }
  ...
  data = {
    database_name = module.rds_2.database_name
    db_identifier = module.rds_2.db_identifier
  }
}
