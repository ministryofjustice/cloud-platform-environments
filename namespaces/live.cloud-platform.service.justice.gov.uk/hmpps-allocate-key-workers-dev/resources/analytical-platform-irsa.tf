module "ap_irsa" {
  ...
  # IRSA configuration
  service_account_name = "hmpps-allocate-key-workers-to-ap-s3"
...
data "aws_iam_policy_document" "ap_access" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:s3:::moj-reg-dev/landing/hmpps-allocate-key-workers-dev/*",
      "arn:aws:s3:::moj-reg-dev/landing/hmpps-allocate-key-workers-dev/"
    ]
  }
}
...
resource "kubernetes_secret" "ap_aws_secret" {
  ...
  data = {
    destination_bucket = "s3://moj-reg-dev/landing/hmpps-allocate-key-workers-dev/"
