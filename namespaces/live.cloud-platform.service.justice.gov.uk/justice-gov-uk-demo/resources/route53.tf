resource "aws_route53_zone" "demo_justice_gov_uk_route53_zone" {
  name = "demo.justice.gov.uk"
...
resource "kubernetes_secret" "route53_zone_sec" {
  ...
  data = {
    zone_id      = aws_route53_zone.demo_justice_gov_uk_route53_zone.zone_id
    name_servers = join("\n", aws_route53_zone.demo_justice_gov_uk_route53_zone.name_servers)
  }
}

# Create an A record in the hosted zone for the CloudFront alias.
# Note, the zone_id parameter is set to the zone_id from the demo.justice.gov.uk route53 zone.
