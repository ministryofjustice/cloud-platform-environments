...
data "github_repository_file" "subscription_filter_policy" {
...
resource "aws_sns_topic_subscription" "event_test_client_subscription" {
  ...
  filter_policy = coalesce(data.github_repository_file.subscription_filter_policy.content, var.default_subscription_filter_policy)
  depends_on = [
    module.hmpps-integration-events
  ]
}
