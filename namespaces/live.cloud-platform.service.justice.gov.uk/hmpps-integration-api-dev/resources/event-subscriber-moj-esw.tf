...
data "aws_secretsmanager_secret" "moj_esw_filter_list" {
  ...
  name = "live-hmpps-integration-api-dev-412962aea4333ad8"
}
...
resource "aws_sns_topic_subscription" "event_moj_esw_subscription" {
  ...
  filter_policy = coalesce(data.github_repository_file.moj_esw_subscription_filter_policy.content, var.default_subscription_filter_policy)
  depends_on = [
    module.hmpps-integration-events
  ]
}
