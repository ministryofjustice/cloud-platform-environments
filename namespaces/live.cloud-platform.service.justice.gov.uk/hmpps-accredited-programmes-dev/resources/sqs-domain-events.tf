...
resource "aws_sqs_queue_policy" "hmpps_acp_domain_events_queue_policy" {
  queue_url = module.hmpps_acp_domain_events_queue.sqs_id

  policy = <<EOF
  {
    "Version": "2012-10-17",
    "Id": "${module.hmpps_acp_domain_events_queue.sqs_arn}/SQSDefaultPolicy",
    "Statement":
      [
        {
          "Effect": "Allow",
          "Principal": {"AWS": "*"},
          "Resource": "${module.hmpps_acp_domain_events_queue.sqs_arn}",
...
resource "kubernetes_secret" "hmpps_acp_domain_events_queue_secret" {
  metadata {
  ...
  data = {
    sqs_queue_url  = module.hmpps_acp_domain_events_queue.sqs_id
    sqs_queue_arn  = module.hmpps_acp_domain_events_queue.sqs_arn
    sqs_queue_name = module.hmpps_acp_domain_events_queue.sqs_name
  }
}

resource "kubernetes_secret" "hmpps_acp_domain_events_queue_secret_dead_letter_queue" {
  metadata {
