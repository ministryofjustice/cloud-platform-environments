data "aws_iam_policy_document" "combined_local_sqs" {
  version = "2012-10-17"
  statement {
    sid     = "hmppsPrisonerPaySqs"
    effect  = "Allow"
    actions = ["sqs:*"]
    resources = [
      module.hmpps_prisoner_pay_api_jobs_queue.sqs_arn,
      module.hmpps_prisoner_pay_api_jobs_dlq.sqs_arn,
    ]
  }
}
...
module "irsa" {
  ...
  service_account_name = "hmpps-prisoner-pay"
