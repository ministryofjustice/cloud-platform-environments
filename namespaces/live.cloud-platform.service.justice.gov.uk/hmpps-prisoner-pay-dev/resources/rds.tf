...
module "prisoner_pay_rds_read_replica" {
  source                      = "github.com/ministryofjustice/cloud-platform-terraform-rds-instance?ref=9.2.0"
  vpc_name                    = var.vpc_name
  team_name                   = var.team_name
  business_unit               = var.business_unit
  ...
  storage_type                = "gp3"
  db_max_allocated_storage    = "50"
  replicate_source_db         = module.prisoner_pay_rds.db_identifier
...
resource "kubernetes_secret" "prisoner_pay_rds_read_replica" {
  metadata {
    name      = "prisoner-pay-read-replica"
    namespace = var.namespace
  }

  data = {
    rds_instance_endpoint = module.prisoner_pay_rds_read_replica.rds_instance_endpoint
    rds_instance_address  = module.prisoner_pay_rds_read_replica.rds_instance_address
  }
}
