module "irsa" {
  ...
  # IRSA configuration
  service_account_name = "${var.namespace}-irsa-sa"
...
module "irsa_service_pod" {
  ...
  role_policy_arns = {
    rds = module.rds.irsa_policy_arn
    # No need for servicepod to have write access to S3, as it lacks read access - all management of S3 is done in the get-payments-finance-data-dev service pod instead.
  }
