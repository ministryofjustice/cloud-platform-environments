resource "aws_cloudwatch_log_group" "api_gateway_log_group" {
  name              = "API-Gateway-Execution-Logs_dces-drc_api_dev"
...
resource "aws_apigatewayv2_integration" "dces_drc_integration_dev" {
  api_id             = aws_apigatewayv2_api.gateway.id
...
resource "aws_apigatewayv2_route" "route" {
  ...
  depends_on = [
    aws_apigatewayv2_api.gateway,
    aws_apigatewayv2_integration.dces_drc_integration_dev,
  ]
}

resource "aws_apigatewayv2_deployment" "deployment" {
  api_id      = aws_apigatewayv2_api.gateway.id
  description = "API Gateway deployment for DCES DRC Integration Service"
