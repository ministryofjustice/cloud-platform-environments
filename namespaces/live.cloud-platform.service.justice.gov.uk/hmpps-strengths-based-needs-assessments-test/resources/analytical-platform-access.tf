...
data "aws_iam_policy_document" "ap_access" {
  ...
  statement {
    ...
    resources = [
      "arn:aws:s3:::moj-reg-dev/landing/hmpps-strengths-based-needs-assessments-test/*",
      "arn:aws:s3:::moj-reg-dev/landing/hmpps-strengths-based-needs-assessments-test/"
    ]
  }
}
...
resource "kubernetes_secret" "ap_aws_secret" {
  ...
  data = {
    destination_bucket = "s3://moj-reg-dev/landing/hmpps-strengths-based-needs-assessments-test/"
