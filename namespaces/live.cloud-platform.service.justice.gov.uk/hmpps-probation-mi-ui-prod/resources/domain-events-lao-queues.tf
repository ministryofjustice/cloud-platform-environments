...
resource "aws_sns_topic_subscription" "hmpps_probation_mi_domain_events_subscription" {
  ...
  protocol  = "sqs"
  endpoint  = module.hmpps_probation_mi_domain_events_queue.sqs_arn
...
resource "kubernetes_secret" "hmpps_probation_mi_domain_events_queue_secret" {
  metadata {
  ...
  data = {
    queue_url  = module.hmpps_probation_mi_domain_events_queue.sqs_id
    queue_arn  = module.hmpps_probation_mi_domain_events_queue.sqs_arn
    queue_name = module.hmpps_probation_mi_domain_events_queue.sqs_name
  }
}

resource "kubernetes_secret" "hmpps_probation_mi_domain_events_queue_secret_dlq" {
  metadata {
  ...
  data = {
    queue_url  = module.hmpps_probation_mi_domain_events_dlq.sqs_id
    queue_arn  = module.hmpps_probation_mi_domain_events_dlq.sqs_arn
...
resource "aws_cloudwatch_metric_alarm" "hmpps_probation_mi_domain_events_dlq_alarm" {
  ...
  alarm_description   = "Messages have been in the lao_domain_events_dlq (prod) for longer than 15 minutes"
  namespace           = "AWS/SQS"
