...
output "non_prod_nlb_ip_addresses" {
  description = "IP addresses of the default NLB - Add these to Ingress Allow List in values-preprod.yaml"
  value = toset(flatten([
...
output "vpc_link_id" {
  ...
  value       = data.aws_api_gateway_vpc_link.shared_vpc_link.id
}
...
output "allowlist_cidr_blocks" {
  description = "CIDR blocks to add to Ingress Allow List (for use in values-preprod.yaml)"
  value = [
