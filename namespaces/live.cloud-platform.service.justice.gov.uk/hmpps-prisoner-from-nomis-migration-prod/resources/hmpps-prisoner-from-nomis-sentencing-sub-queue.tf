module "prisoner_from_nomis_sentencing_queue" {
  ...
  redrive_policy = jsonencode({
    deadLetterTargetArn = module.prisoner_from_nomis_sentencing_dead_letter_queue.sqs_arn
    maxReceiveCount     = 3
  })
...
resource "kubernetes_secret" "prisoner_from_nomis_sentencing_queue" {
  metadata {
    name      = "prison-events-sentencing-queue"
    namespace = var.namespace
  }
...
resource "kubernetes_secret" "prisoner_from_nomis_sentencing_dead_letter_queue" {
  metadata {
    name      = "prison-events-sentencing-dl-queue"
    namespace = var.namespace
  }
