...
module "read_replica-1" {
  ...
  db_name = module.rds-2.database_name

  # Set the db_identifier of the source db
  replicate_source_db = module.rds-2.db_identifier
...
resource "kubernetes_secret" "rds-2" {
  metadata {
  ...
  data = {
    rds_instance_endpoint = module.rds-2.rds_instance_endpoint
    database_name         = module.rds-2.database_name
    database_username     = module.rds-2.database_username
    database_password     = module.rds-2.database_password
    rds_instance_address  = module.rds-2.rds_instance_address
  }
...
resource "kubernetes_secret" "read_replica-2" {
  # default off
  count = 0

  metadata {
    name      = "rds-postgresql-read-replica-output-2"
    namespace = var.namespace
  }
...
resource "kubernetes_config_map" "rds-2" {
  metadata {
    name      = "rds-postgresql-instance-output-2"
    namespace = var.namespace
  }
  ...
  data = {
    database_name = module.rds-2.database_name
    db_identifier = module.rds-2.db_identifier
