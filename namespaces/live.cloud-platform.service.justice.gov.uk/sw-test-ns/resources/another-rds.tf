...
module "rds-test" {
  ...
  vpc_security_group_ids = [aws_security_group.another-rds-sg.id]
...
resource "kubernetes_secret" "read_replica-test" {
  # default off
  count = 0
...
resource "aws_security_group" "another-rds-sg" {
  name        = "rds-custom-sg-group"
