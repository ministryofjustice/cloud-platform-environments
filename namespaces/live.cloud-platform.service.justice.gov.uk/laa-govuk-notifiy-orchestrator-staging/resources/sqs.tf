module "laa_govuk_notify_orchestrator_staging_sqs" {
  ...
  # Queue configuration
  # The queue is called "laa_govuk_notify_orchestrator_staging_queue.fifo",
  ...
  redrive_policy = <<EOF
  {
    "deadLetterTargetArn": "${module.laa_govuk_notify_orchestrator_staging_dlq.sqs_arn}","maxReceiveCount": 1
  }
  EOF
}
