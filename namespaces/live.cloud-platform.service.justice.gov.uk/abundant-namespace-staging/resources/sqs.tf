...
resource "kubernetes_secret" "abundant_staging_sqs" {
  metadata {
    name      = "sqs"
    namespace = var.namespace
  }
  ...
  data = {
    access_key_id     = module.abundant_staging_sqs.access_key_id
    secret_access_key = module.abundant_staging_sqs.secret_access_key
    # the above will not be set if existing_user_name is defined
    sqs_id   = module.abundant_staging_sqs.sqs_id
    sqs_arn  = module.abundant_staging_sqs.sqs_arn
    sqs_name = module.abundant_staging_sqs.sqs_name
  }
}
