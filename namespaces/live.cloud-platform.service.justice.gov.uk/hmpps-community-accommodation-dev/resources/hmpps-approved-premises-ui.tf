...
# Note, redis is a requirement for hmpps-template-typescript application. # Q - is this important?
module "elasticache_redis" {
  source                 = "github.com/ministryofjustice/cloud-platform-terraform-elasticache-cluster?ref=7.2.0"
  vpc_name               = var.vpc_name
  team_name              = var.team_name
  business_unit          = var.business_unit
  application            = module.hmpps_approved_premises_iu.application
  is_production          = var.is_production
  namespace              = var.namespace
...
resource "kubernetes_secret" "elasticache_redis" {
  metadata {
    name      = "${module.hmpps_approved_premises_iu.application}-elasticache-redis"
    namespace = var.namespace
  }
