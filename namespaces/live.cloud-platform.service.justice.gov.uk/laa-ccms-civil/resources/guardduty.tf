...
resource "aws_guardduty_malware_protection_plan" "malware_bucket1_plan" {
  role = aws_iam_role.guardduty_malware_protection_role.arn
    ...
    tags = {
        Name = "GuardDutyMalwareProtectionPlan-${module.laa_ccms_documents.bucket_name}"
    }
}

resource "aws_iam_role" "guardduty_malware_protection_role" {
  name = "${var.namespace}-guardduty-malware-protection-role"
  path = "/"
...
resource "aws_iam_policy" "guard-duty_s3_custom" {
  name        = "${var.namespace}-guard-duty-s3-policy"
  ...
  policy = jsonencode({
    ...
    Statement = [
      ...
      },
      {
        ...
        Resource = [
          "arn:aws:s3:::${module.laa_ccms_documents.bucket_name}/*"
        ]
        Condition = {
          StringEquals = {
      ...
      },
      {
        ...
        Resource = [
          "arn:aws:s3:::${module.laa_ccms_documents.bucket_name}/malware-protection-resource-validation-object"
        ]
        Condition = {
          StringEquals = {
            "aws:ResourceAccount" = data.aws_caller_identity.current.account_id
