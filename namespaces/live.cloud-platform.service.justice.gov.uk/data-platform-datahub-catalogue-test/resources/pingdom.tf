...
resource "pingdom_check" "datahub-catalogue-test" {
  for_each                 = local.urls
  type                     = "http"
  name                     = "datahub-catalogue - test - ${each.key} - cloud-platform"
  host                     = "datahub-catalogue-test.apps.live.cloud-platform.service.justice.gov.uk"
  resolution               = 1
