...
data "aws_iam_policy_document" "data_compliance_prod_ap_policy" {
  statement {
    actions = [
      "sts:AssumeRole"
    ]
...
resource "aws_iam_user" "user" {
  name = "data-compliance-prod-ap-user-${random_id.id.hex}"
...
resource "aws_iam_user_policy" "policy" {
  ...
  policy = data.aws_iam_policy_document.data_compliance_prod_ap_policy.json
  user   = aws_iam_user.user.name
}
