module "irsa" {
  ...
  # Policy ARNs
  role_policy_arns = {
    s3 = aws_iam_policy.migration-link-exchange-build-prod-s3-policy.arn
  }
...
data "aws_iam_policy_document" "migration-link-exchange-build-prod-s3-policy" {
...
resource "aws_iam_policy" "migration-link-exchange-build-prod-s3-policy" {
  name   = "migration-link-exchange-build-prod-s3-policy"
  policy = data.aws_iam_policy_document.migration-link-exchange-build-prod-s3-policy.json
...
data "aws_ssm_parameter" "s3-bucket-name" {
  name = "/migration-link-exchange-prod/s3-bucket-name"
}

data "aws_ssm_parameter" "s3-bucket-arn" {
  name = "/migration-link-exchange-prod/s3-bucket-arn"
}
