...
resource "aws_iam_user_policy" "dces-ca-admin_policy" {
  name   = "laa-dces-data-integration-uat-ca-admin-policy"
  policy = data.aws_iam_policy_document.dces_ca_admin_policy.json
  user   = aws_iam_user.dces_ca_admin_user_uat.name
}

resource "aws_iam_user" "dces_ca_admin_user_uat" {
  name = "laa-dces-drc-integration-uat-dces-ca-admin_user"
  path = "/system/laa-dces-data-integration-uat-dces-ca-admin_user/"

}

resource "aws_iam_access_key" "dces_ca-admin_user_uat_key" {
  user = aws_iam_user.dces_ca_admin_user_uat.name
}
