module "opensearch_alert" {
    ...
    secret_name                    = "live-grc-preprod-9fe15ba82682abad"
    ...
    opensearch_alert_name          = "grc-preprod-bots"
    opensearch_alert_enabled       = true
    monitor_period_interval        = "30"
    monitor_period_unit            = "MINUTES"
    index                          = ["live_kubernetes_ingress-*"]

    trigger_name                   = "grc-preprod-bots"
    serverity                      = "1"
    query_source                   = "ctx.results[0].hits.total.value > 5"
    action_name                    = "grc-preprod-send-alert"
    slack_message_subject          = "GRC Pre-prod Bot and DOS Alert"
    slack_message_template         = "GRC Preprod Monitor {{ctx.monitor.name}} just entered alert status. Please investigate the issue.\n- Trigger: {{ctx.trigger.name}}\n- Severity: {{ctx.trigger.severity}}"
                ...
                },
                {
                  "bool": {
                    "should": [
                      {
                        "match_phrase": {
                          "log_processed.kubernetes_namespace": "grc-preprod"
                        }
                      }
                    ],
