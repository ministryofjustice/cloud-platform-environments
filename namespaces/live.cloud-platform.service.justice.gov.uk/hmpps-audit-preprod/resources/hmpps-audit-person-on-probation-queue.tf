module "hmpps_person_on_probation_audit_queue" {

  source = "github.com/ministryofjustice/cloud-platform-terraform-sqs?ref=5.1.2"

  # Queue configuration
  sqs_name                  = "hmpps_person_on_probation_audit_queue"
  encrypt_sqs_kms           = "true"
  message_retention_seconds = 1209600

  redrive_policy = <<EOF
  {
    "deadLetterTargetArn": "${module.hmpps_person_on_probation_audit_dead_letter_queue.sqs_arn}","maxReceiveCount": 3
  }
...
module "hmpps_person_on_probation_audit_dead_letter_queue" {
  source = "github.com/ministryofjustice/cloud-platform-terraform-sqs?ref=5.1.2"

  # Queue configuration
  sqs_name        = "hmpps_person_on_probation_audit_dlq"
  encrypt_sqs_kms = "true"
...
resource "kubernetes_secret" "hmpps_person_on_probation_audit_queue_secret" {
  metadata {
    name      = "sqs-person-on-probation-audit-queue-secret"
    namespace = var.namespace
  }

  data = {
    sqs_queue_url  = module.hmpps_person_on_probation_audit_queue.sqs_id
    sqs_queue_arn  = module.hmpps_person_on_probation_audit_queue.sqs_arn
    sqs_queue_name = module.hmpps_person_on_probation_audit_queue.sqs_name
  }
}
...
resource "kubernetes_secret" "hmpps_person_on_probation_audit_dead_letter_queue_secret" {
  ...
  data = {
    sqs_queue_url  = module.hmpps_person_on_probation_audit_dead_letter_queue.sqs_id
    sqs_queue_arn  = module.hmpps_person_on_probation_audit_dead_letter_queue.sqs_arn
