...
module "hmpps_registers_to_nomis_queue" {
  ...
  sqs_name                  = "hmpps_registers_to_nomis_queue"
  ...
  redrive_policy = <<EOF
  {
    "deadLetterTargetArn": "${module.hmpps_registers_to_nomis_dead_letter_queue.sqs_arn}","maxReceiveCount": 3
  }
...
resource "aws_sqs_queue_policy" "hmpps_registers_to_nomis_queue_policy" {
  queue_url = module.hmpps_registers_to_nomis_queue.sqs_id

  policy = <<EOF
  {
    "Version": "2012-10-17",
    "Id": "${module.hmpps_registers_to_nomis_queue.sqs_arn}/SQSDefaultPolicy",
    "Statement":
      [
        {
          "Effect": "Allow",
          "Principal": {"AWS": "*"},
          "Resource": "${module.hmpps_registers_to_nomis_queue.sqs_arn}",
...
module "hmpps_registers_to_nomis_dead_letter_queue" {
  ...
  infrastructure-support = var.infrastructure_support
  application            = var.application
  sqs_name               = "hmpps_registers_to_nomis_dlq"
  encrypt_sqs_kms        = "true"
  namespace              = var.namespace
...
resource "aws_sns_topic_subscription" "hmpps_registers_to_nomis_subscription" {
  ...
  protocol      = "sqs"
  endpoint      = module.hmpps_registers_to_nomis_queue.sqs_arn
