module "domain_events_queue" {
  ...
  team_name              = "hmpps-csip" # team name is used in queue name so setting to hard coded value
...
# Below this is to be removed in a future PR

module "hmpps_csip_domain_events_queue" {
  source = "github.com/ministryofjustice/cloud-platform-terraform-sqs?ref=5.1.0"

  sqs_name                  = "hmpps_csip_domain_events_queue"
  redrive_policy = jsonencode({
    deadLetterTargetArn = module.hmpps_csip_domain_events_dlq.sqs_arn
    maxReceiveCount     = 3
  })
  ...
  application            = var.application
  is_production          = var.is_production
  team_name              = "hmpps-move-and-improve" # maintain existing value until app switched
  environment_name       = "development" # maintain existing value until app switched
  namespace              = var.namespace
  infrastructure_support = var.infrastructure_support
}

resource "aws_sqs_queue_policy" "hmpps_csip_domain_events_queue_policy" {
  queue_url = module.hmpps_csip_domain_events_queue.sqs_id
  policy    = data.aws_iam_policy_document.sqs_queue_policy_document.json
}

module "hmpps_csip_domain_events_dlq" {
  source = "github.com/ministryofjustice/cloud-platform-terraform-sqs?ref=5.1.0"

  sqs_name        = "hmpps_csip_domain_events_dlq"
  message_retention_seconds = 7 * 24 * 3600 # 1 week
...
resource "kubernetes_secret" "hmpps_csip_domain_events_queue_secret" {
  metadata {
...
resource "kubernetes_secret" "hmpps_csip_domain_events_queue_secret_dlq" {
  metadata {
