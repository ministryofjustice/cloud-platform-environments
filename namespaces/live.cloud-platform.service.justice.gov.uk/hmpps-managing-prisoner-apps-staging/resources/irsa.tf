# Combined policy for locally created SQS queues
data "aws_iam_policy_document" "combined_local_sqs" {
  version = "2012-10-17"
  statement {
    sid     = "hmppsManagingPrisonerAppsSqs"
    effect  = "Allow"
    actions = ["sqs:*"]
    resources = [
      module.managing_prisoner_apps_domain_events_queue.sqs_arn,
      module.managing_prisoner_apps_domain_events_dlq.sqs_arn,
    ]
  }
}

resource "aws_iam_policy" "combined_local_sqs" {
  name   = "hmpps-managing-prisoner-apps-staging-combined-local-sqs"
