...
module "datahub_query_metrics_assume_role" {
  source                         = "terraform-aws-modules/iam/aws//modules/iam-assumable-role-with-oidc"
  version                        = "5.60.0"
  create_role                    = true
  role_name                      = "datahub-query-metrics-${var.environment}-cross-iam-${var.eks_cluster_name}"
  ...
  oidc_fully_qualified_subjects  = ["system:serviceaccount:${var.namespace}:datahub-query-metrics-${lookup(local.environments_map, lower(var.environment))}-cross-iam"]
...
resource "aws_iam_policy" "cross_iam_dpr_oidc" {
  name   = "${var.namespace}-cross-iam-oidc"
...
module "irsa" {
  ...
  service_account_name = "datahub-query-metrics-${var.environment}-cross-iam"
