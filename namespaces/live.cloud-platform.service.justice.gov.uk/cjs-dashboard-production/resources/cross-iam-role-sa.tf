...
resource "aws_iam_policy" "cjs_dashboard_prod_ap_policy" {
  name   = "cjs_dashboard_prod_ap_policy"
  policy = data.aws_iam_policy_document.cjs_dashboard_prod_ap_policy.json
...
resource "aws_iam_user" "cjs_dashboard_prod_ap_user" {
  name = "ap-s3-user-${random_id.cjs-dashboard-prod-ap-id.hex}"
  path = "/system/cjs-dashboard-prod-ap-s3-bucket-user/"
}

resource "aws_iam_access_key" "cjs_dashboard_prod_ap_user" {
  user = aws_iam_user.cjs_dashboard_prod_ap_user.name
}

resource "aws_iam_user_policy" "cjs_dashboard_prod_ap_policy" {
...
resource "kubernetes_secret" "ap_aws_secret" {
  metadata {
    name      = "cjs-dashboard-prod-ap-s3-bucket"
    namespace = var.namespace
  }
